...
Current task: **Task 16 — Search persistence (query in the URL so Back preserves results).**
- What: pushing the current query to the URL (e.g. `/?q=training+frequency`) so the browser Back button restores the previous search + results, and a search from a study page returns to the same result list.
- Why: currently a natural-language search's results are lost the moment the user navigates to a study and presses Back — the page resets to an empty search state. This is a Phase 2 polish item from the roadmap queue (#3) that makes the Explorer feel like a real research assistant.
- Deliverable: search state (entered query, and where sensible the translated query + results) survives navigation; the URL reflects the query; Back preserves results.
- Notes: keep the existing "rank, don't filter" behavior; consider `useSearchParams` (Next.js 16 — it's a client hook, wraps in `Suspense`) or a server-rendered initial state; the AI translation itself should not need to re-run on every Back nav (cache/derive from the URL query; re-running DeepSeek on every Back press would be wasteful + slow); follow the read `PROJECT_NOTES.md` + §12.3 → implement → `tsc --noEmit` → `npm run build` → commit + push → define Task 17 loop.
...
3. [ ] Search persistence (Task 16) - push the query to the URL so Back preserves results.
