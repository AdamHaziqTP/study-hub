...
Current task: **Task 17 — Pagination / Load More (remove the hardcoded 10-study limit).**
- What: a search currently returns at most 10 results with no way to see more. Add a "Load more" control that fetches the next page and appends it to the existing results.
- Why: Phase 2 roadmap queue item (#4); the Explorer should feel like a real research tool, and a topical search ("hypertrophy training frequency") legitimately returns dozens of relevant studies the user can't reach today.
- Deliverable: search results paginate — a "Load more" button (or infinite scroll) appends the next page; the URL query keeps reflecting the search (`/?q=...`); loading/error/end-of-results states handled.
- Notes: keep the existing "rank, don't filter" behavior; the EXISTING `searchPubMed(term, retmax)` already accepts `retmax` and `/api/ai-search` already forwards it — extend to `retstart`/`page` so ESearch offsets into the ranked list instead of re-fetching the same top 10; the AI-translated query must be REUSED for page 2+ (never re-translate deep in the list); cache reconciliation with Task 16's `resultsCache` (append to, or extend, the cached payload); follow the read `PROJECT_NOTES.md` + §12.3 → implement → `tsc --noEmit` → `npm run build` → commit + push → define Task 18 loop (next roadmap item #5 — Sticky search header).
