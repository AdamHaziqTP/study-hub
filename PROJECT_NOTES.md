...
| `src/app/study/[pmid]/page.tsx` | Server page: DB-first (loads `studies` + saved `study_context`/limitations + `study_simplifications`), falls back to live PubMed; passes `studyId` to `<StudyDetail>`; `force-dynamic` so save-state stays fresh | Saved context + simplification render without an AI call on revisit; `studyId` unlocks PersonalNotes; Next.js 16 `params`-as-`Promise` |
...
| `src/lib/ai.ts` | Server-only DeepSeek helper: `extractStudyContext(title, abstract, fullText?)` → validated `StudyContext` + `IdentifiedLimitation[]`; **`simplifyStudy(title, abstract, fullText?)` → `StudySimplification` (Job 2); `assessClaimAlignment({claimText, studies})` → `ClaimAlignment` (verdict + reasoning); shared `getModel()`/`callDeepSeek()` — ONE cheap model (`deepseek-chat` default) for ALL jobs, NO heavier-model logic** | `deepseek-chat` default; strict JSON validation; two-tier limitations |
...
6. **Evidence context** — placeholder for the factor explanations (no credibility score)
7. **What this might mean for training** — placeholder for cautious practical interpretation
...
- ⏳ **Action required by user:** run the updated `sql/schema.sql` in Supabase ONCE (adds `source_info` column, `study_identified_limitations` table + index + regenerable RLS policies, the `study_notes` table + per-user RLS + FK indexes, the Task 7 migration that recreates `articles`/`claims`/`evidence_links` with `user_id DEFAULT auth.uid()`, per-user RLS + GRANTs, FK/RLS indexes — AND the Task 9 `study_simplifications` table + regenerable RLS) so context persistence, personal notes, the article editor, the evidence graph AND the "In plain English" block all work end-to-end
...
10. **Polish → deploy → test → docs (Task 10)** — see §12.2.
...
- **Cheap model default (Task 9):** `DEEPSEEK_MODEL` defaults to `deepseek-chat` in `src/lib/ai.ts`. ALL THREE AI jobs (extract, simplify, claim alignment) share the SAME cheap fast model via the shared `getModel()` — there is deliberately **no** separate heavier-model logic.
...
**Task 10 (do this next): Polish → deploy → test → docs.**
- What: wrap up the product for the portfolio: (a) **Polish** the remaining placeholder sections (`Evidence context` and `What this might mean for training` in `StudyDetail.tsx` — Job 3 cautions, plain-language "why each factor matters", no credibility score) and any rough UI edges; (b) **Verify end-to-end** in the browser with the test PMIDs once the schema (Task 3/6/7/9) is applied in Supabase + GitHub OAuth enabled; (c) **Deploy** to Vercel (env: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `DEEPSEEK_API_KEY`) and re-verify the deployed flows + Supabase Redirect URLs include the prod URL; (d) **Docs**: finish this file's status, verify `README.md` covers setup/auth/schema, and record any deployment gotchas.
- Product fit: the app's core promise (find → understand → interpret → conclude) needs the two placeholder sections alive; deployment makes it a real portfolio artifact reachable by an interviewer.
- Head start: `StudyDetail.tsx` already has the two placeholder sections plus the regenerable patterns (context + simplification) to mirror; `.env.example` documents all env vars; the graph/articles/notes features are built and just need schema + auth applied.
- Deliverable: (a) functional `Evidence context` + `What this might mean for training` sections on `/study/[pmid]`; (b) a live deployed URL on Vercel with `tsc --noEmit` + `npm run build` clean; (c) updated `PROJECT_NOTES.md` + `README.md`.
- Notes: keep it tight — polish only what makes the core story coherent at interview time. **Reminder: apply the accumulated `sql/schema.sql` (Task 3 + Task 6 + Task 7 + Task 9) in Supabase and enable GitHub OAuth BEFORE browser/deploy verification, since the persisted features depend on them.**
...
- Git: **Task 9 committed and pushed** to `main`.
...
- Supabase project is live (don't re-run full schema unless asked; `sql/schema.sql` is source of truth — but the Task 3 + Task 6 + **Task 7** + **Task 9** additions DO need to be applied once for context persistence, notes, articles, the evidence graph AND simplification: `source_info` column, `study_identified_limitations` table + index + regenerable RLS, `study_notes` table + per-user RLS + FK indexes, the Task 7 migration which DROPs + recreates `articles`/`claims`/`evidence_links` empty with `user_id DEFAULT auth.uid()`, per-user RLS policies + GRANTs, and the Task 9 `study_simplifications` table + regenerable RLS).
