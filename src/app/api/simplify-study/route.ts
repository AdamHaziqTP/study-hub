...
import { simplifyStudy } from "@/lib/ai";

/**
 * POST /api/simplify-study
 *
 * Job 2 ("Explain"): generates a plain-English re-explanation of a study
 * for a curious lifter. Mirrors /api/extract-context — no DB write.
 *
 * Body (JSON):
 *   { "pmid": "35819335" }               -> fetches from PubMed (+ PMC full text when available), simplifies
 *   { "title": "...", "abstract": "..." } -> simplifies directly from provided text (abstract-only)
 *
 * Response: { study, simplification, sourceInfo }
 *   - study          = the raw PubMedStudy (source, unmodified)
 *   - simplification = validated StudySimplification from DeepSeek
...
export async function POST(request: NextRequest) {
  try {
    ...
    const simplification = await simplifyStudy({
      title: studyTitle,
    ...
    return NextResponse.json({ study, simplification, sourceInfo });
  } catch (err) {
    console.error("simplify-study error:", err);
    return NextResponse.json(
